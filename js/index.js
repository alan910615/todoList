...
document.addEventListener("DOMContentLoaded", function(){
  ...
  button_task_add.addEventListener("click", function(){
    ...
    let task_date = document.getElementsByClassName("date_picker")[0].value;
    ...
    var fullDate = year + "-" + (month < 10 ? `${"0" + (month + 1)}` : (month + 1)) + "-" + (day < 10 ? `${"0" + day}` : day);
    var todo_date = document.getElementsByClassName('todo_date');
    for(let i = 0; i < todo_date.length; i++){
      if(todo_date[i].innerText == fullDate){
        todo_date[i].classList.add('red');
      }
    }
  });
